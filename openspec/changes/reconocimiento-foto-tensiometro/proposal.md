...
- **Cliente Vision API de Gemini**: Envío directo desde el navegador (fetch nativo) de la imagen comprimida junto con un prompt estructurado para extraer sistólica, diastólica y pulso.
...
- **Código**: Nuevos componentes o vistas para configuración de clave API y captura/recorte/procesamiento de imagen en el cliente (`src/services/gemini.ts`, `src/components/ApiKeyModal.tsx`, `src/components/CameraCaptureModal.tsx`, `src/components/PhotoConfirmModal.tsx`).
